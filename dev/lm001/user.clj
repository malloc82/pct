...
(let [mem (let [[s u] (pct.util.system/readableFormat pct.util.system/MaxMemory)]
  ...
  (taoensso.timbre/info " >>>>>>>>>>>>>>> * dev/lm001/user.clj loaded * <<<<<<<<<<<<<<<"  "")
...
(let [base-dir "datasets/data_4_Ritchie_10_24_2019"
  ...
  (if (.isDirectory f)
    (do (def dataset_1 (pct.data.io/newPCTDataset
        ...
        (def res (pct.data.io/load-dataset dataset_1 {:min-len 15 :batch-size 70000})))
    (taoensso.timbre/info (format "Path [%s] does not exist or is not a folder." base-dir))))
